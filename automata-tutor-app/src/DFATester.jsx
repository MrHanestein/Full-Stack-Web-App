...
export default function DFATester({ states, transitions, startState }) {
    ...
    const runSimulation = () => {
        ...
        // example of highlight path (if user added id to circles)
    ...
    // If admin user, load all user-submitted DFAs from Firestore
        ...
        <div className="mt-8 p-4 border rounded bg-gray-50">
            ...
            {result && (
                <div className="text-sm mt-2">
                    ...
                    <p className="mt-1">
                        Path Traversed: <code>{result.path.join(" → ")}</code>
                    </p>
                </div>
            )}
            ...
            {currentUser?.email === "admin@example.com" && (
                <div className="mt-8 border-t pt-4">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                        Viewing saved DFAs from all users:
                    </p>
                    <ul className="text-sm space-y-1">
                        {userDFAs.map((entry, i) => (
                            <li key={i} className="bg-white border px-2 py-1 rounded">
                                <strong>User:</strong> {entry.id}<br />
                                <strong>DFA States:</strong> {entry.dfa?.states?.length || 0} |{" "}
                                <strong>Transitions:</strong> {entry.dfa?.transitions?.length || 0}
                            </li>
                        ))}
                    </ul>
                    <div className="mt-4">
